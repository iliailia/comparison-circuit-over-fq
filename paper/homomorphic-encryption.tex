...
The former one corresponds to non-scalar multiplication of two expressions containing input values, while the latter is related to the product of an expression with input values and an unenecrypted finite field element. 
Since $\Mul$ is much more expensive than $\MulPlain$, the multiplicative depth of a homomorphic circuit is usually calculated with relation to the number of $\Mul$'s, or non-scalar multiplications.
