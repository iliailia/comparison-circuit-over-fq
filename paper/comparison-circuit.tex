As explained in Section \ref{sec:background}, the comparison of elements over $\mathbb{F}_{p^d}$ boils down to their comparison over $\mathbb{F}_p$. 
...
Following the idea of \cite{TLWRK20} and using the decomposition of $f(X,Y)$ given in Eq. (\ref{eq:decomposition-f-final}) one needs:
\begin{itemize}
...
\end{align*}
  ...
  \end{align*}
  The following lemma describes all the coefficients of $Q_{\LT_\S}(X,Y)$.
  ...
  \end{proof}
  \textbf{Complexity analysis.} The above lemma implies that the less-than function can be expressed by a univariate polynomial of degree $p-1$.
  ...
  \begin{remark}
    ...
    Thus, we can get $\EQ_\S(X,Y)$ almost for free (at the cost of one $\SubPlain$) after evaluating $\LT_\S(X,Y)$, thus saving $O(\log (p-1))$ non-scalar multiplications.
    ...
    This saves $O((d'-1)(k-1) \log (p-1))$ homomorphic multiplications, thus leading to a better running time than for the bivariate circuit.\todo{Mention the fact that since one can only use half of the plaintext space, univariate method cannot really be used if one has to compute stuff before the comparison}
  \end{remark}
