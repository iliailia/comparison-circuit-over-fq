...
\subsection{Min/max function}
  ...
  In this section, we summarize the complexities of our method as compared to the work of Tan et al for the evaluation of the less-than function using the lexicographical order method described in Section \ref{sec:background}. Note that our algorithms do not improve the depth of the circuit, as so we only compare the number of homomorphic multiplications required by the different methods.
  
  With fixed $p$ and $d$, the previous work of Tan et al. \cite{TLWRK20} (Section 4.4) showed that in the case of the bivariate circuit the less-than function of two $b$-bit integers requires:
  
  $$d\cdot\left(T+ \lceil\log_2(p-1)\rceil + \lceil \log_2 d \rceil \right) + \lfloor\log_2(\log_p 2^b)/d)\rfloor+2$$

  homomorphic multiplications.

  where $T$ is the number of homomorphic multiplications required to evaluate the comparison circuit over $\mathbb{F}_p$. In the work of Tan et al. $T = 3p-5$, while in our case $T = 2p-6$. \textcolor{red}{Actually it does not seem that Tan et al. took into account the equality circuit in their analysis otherwise they should have $4p - 6$ mult: $2p-4$ to compute the powers of $x$ and $y$, $p-1$ to compute $\LT_\S$ and another $p-1$ (from what they say, $\log_2(p-1)$ otherwise), to get $\EQ_\S$. }

  The univariate method allows to save even more multiplications since when computing $\LT_\S(x,y)$ we compute $\texttt{EQ}_\S(x,y)$ and so we have it for free when evaluating the lexicographical order (Remark 2). So in this case the comparison of two $b$-bits integers requires:

  $$\textcolor{red}{TODO}$$
  
  with $T\approx \sqrt{2p-4}+3(\log_2(2p-4))/2$ in this case.
