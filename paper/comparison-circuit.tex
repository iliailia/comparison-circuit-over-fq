...
\subsection{Min/max function}
  ...
  In this section, we summarize the complexities of our method as compared to the work of Tan et al for the evaluation of the less-than function using the lexicographical order method described in Section \ref{sec:background}. Note that our algorithms do not improve the depth of the circuit which is

  $$ \lfloor \log_2 d \rfloor + \lfloor \log_2 (p-1) \rfloor + \lfloor \log_2 (\log_p 2^b)/d) \rfloor + 4. $$
  
  With fixed $p$ and $d$, Tan et al. determined in \cite{TLWRK20} (Section 4.4), that the number of homomorphic multiplication required to evaluate the less-than function of two $b$-bit integers with the bivariate interpolation is
