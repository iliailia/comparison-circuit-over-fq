...
To evaluate $\LT_S$ as a univariate polynomial, we compute the difference $x-y$ of the two input values and looking at its sign.
...
\end{align*}
  ...
  We can use these powers to compute the leading term in $r$ steps, namely
  \begin{align*}
  ...
  \begin{remark}
    ...
    The downside of the univariate circuit is that only half of the plaintext space is used to encode input integers in comparison to the bivariate method.
    \todo{Mention the fact that since one can only use half of the plaintext space, univariate method cannot really be used if one has to compute stuff before the comparison}
  \end{remark}
