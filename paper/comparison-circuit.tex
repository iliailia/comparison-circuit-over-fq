...
Thus overall, for $p\geq 5$, one needs at most $2p-5$ non scalar multiplications to evaluate $P_{\LT_\S}(x,y)$ homomorphically. Actually by optimising the way of computing the $f_i$ this number can be slightly reduced. For instance for $p=5$ and $7$ it can be done using only $4$ and $6$ multiplications respectively (see Appendix \todo{ref}), hence for a prime $2\leq p \leq 7$, $P_{\LT_\S}(x,y)$ can be computed with only $p-1$ non-scalar multiplications.
