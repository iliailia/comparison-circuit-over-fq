...
For example, $\S$ can be $\{0,1,2,\dots,q-1\}$ if $q$ is a prime number.

Let $p$ be a prime number ans $\S = \{0,1,\dots,p-1\}$.
...
\end{lemma}
\begin{proof}
  It is enough to prove that the sum $\sum_{a=0}^{p-1} a^n = 0 \mod p$ for any $n < p-2$.
...
\todo{the paragraph below as well as Algorithm~\ref{alg:basic_less_than_circuit} might be obsolete}
