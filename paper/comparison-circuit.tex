...
\subsection{Min/max function}
  ...
  The univariate method allows to save even more multiplications since when computing $\LT_\S(x,y)$ we almost compute $\texttt{EQ}_\S(x,y)$ and so we have it almost for free when evaluating the lexicographical order (Remark 2). Thus, in this case the comparison of two $b$-bits integers requires
  $$ d\cdot\left(T + \lceil \log_2 d \rceil \right) + \lfloor\log_2(\log_p 2^b)/d)\rfloor+2. $$
  with $T\approx \sqrt{2p-4}+3(\log_2(2p-4))/2$ by using Paterson-Stockmeyer algorithm.
