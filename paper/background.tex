...
\subsubsection{Comparison of vectors}
  Let $\F_{p^d} = \F_p[X]/\ideal{f(X)}$ for some irreducible monic polynomial $f(X)$ of degree $d$.
  Let us consider the situation where we want to compare two elements $a,b$ of $\F_{p^d}$ of the form $a = \sum_{i=0}^{d'-1} a_i X^i$ and $b = \sum_{i=0}^{d'-1} b_i X^i$ with $a_i, b_i \in \F_p$, i.e. $a$ and $b$ lie in the vector subspace $\F_{p}^{d'}$ of $\F_{p^d}$.

  How to compare such elements?
  Indeed, the order of set $\S$ defines a polynomial function in $\F_{p^d}$, which can be interpolated by Lemma~\ref{lem:interpolation}.
  However, the degree of this polynomial is at most $p^d-1$, which means that the complexity of this method of comparing elements of $\F_{p}^{d'}$ is independent of $d'$ and exponential in $d$.
  ...
  Given the coefficients $a_i, b_i \in \F_p$, we can combine them using the lexicographical order $\bm{<}$ on $\F_{p}^{d'}$ defined by:
   \begin{align*}
      a \bm{<} b \Leftrightarrow \exists i\in[0,d'-1] \text{ such that } a_i < b_i \text{ and } \forall j > i ~~ a_j = b_j\,,
    \end{align*}
    which results in the following circuit
    \begin{align*}
      \LT_{\F_p^{d'}}(a, b) = \sum_{i=0}^{d'-1} \LT_{\F_{p}}(a_{i}, b_{i}) \prod_{j=i+1}^{d'-1} \EQ_{\F_{p}}(a_{j}, b_{j}).
    \end{align*}
  This function requires $d'$ evaluations of $\LT_{\F_{p}}$, $d'$ evaluations of $\EQ_{\F_{p}}$ and $2(d'-1)$ additional multiplications.
  If we ignore the complexity of digit extraction, the total complexity of comparing elements of $\F_{p}^{d'}$ is $O(d'(p-1))$.

\subsubsection{Comparison of large integers}
  %For any $d'\le d$, one can encode integers in $[0,p^{d'}-1]$ as elements of the vector subspace $\F_{p}^{d'}$ of $\F_{p^d}$ and use Lemma \ref{lem:interpolation} to compare them using only additions, substractions and multiplications in $\F_{p^{d'}}$.  

  To compare integers larger than the size of the field $p^d$, one can decompose them in base $\F_{p}^{d'}$, for some $d' \le d$. 
  In this way, we can represent an integer $x = x_0 + x_1p^{d'} + x_2p^{2d'}+\ldots x_{k-1}p^{(k-1)d'}$ by the vector $\vx=(x_0,x_1,\ldots,x_{k-1}) \in \left(\F_{p}^{d'}\right)^k$ of its digits of length $k$.

  From there, the comparison of two integers $x$ and $y$ is equivalent to the comparison of their vector of digits $\vx=(x_0,x_1,\ldots,x_{k-1})$ and $\vy=(y_0,y_1,\ldots,y_{k-1})$ using the lexicographical order $\bm{<}$ on $\left(\F_{p}^{d'}\right)^k$ defined as follows
    \begin{align*}
