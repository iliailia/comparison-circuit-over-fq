...
Thus, each ciphertext encrypts exactly $k = \#slots/l$ integers. 
%%%%%%%%
...
\subsection{Comparison to other HE schemes}
    ...
    Our implementation runs with $p=131$, $m=17293$ ($\phi(m)=17292$), which corresponds to 5764 integers of 8-16 bits or 2882 20-bit integers encoded into one ciphertext.
    ...
    \begin{table}[h]
      ...
      \begin{tabular*}{.45\textwidth}{ p{1.2cm} p{2.1cm} p{1.0cm} p{2cm}}
        ...
        \bottomrule
      \end{tabular*}
      \caption{Total and amortized running time of the less-than function implemented with different HE schemes including TFHE, CKKS and BGV. Note that TFHE does not support SIMD packing, which implies that the total and amortized running time of this scheme are the same. The encryption parameters of each scheme are set to support the following security levels: 110 bits for TFHE, 128 bits for CKKS and 126 bits for BGV.
